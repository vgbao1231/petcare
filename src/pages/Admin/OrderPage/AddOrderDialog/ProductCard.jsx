...
const ProductCard = ({ product, selected, setSelectedProducts }) => {
    ...
    const { id, productType, imgUrl, name, description, price, quantity } = product;

    const addProduct = () => {
        const checkExistedItem = (item) => item.id === id && item.productType === productType;
    ...
    const removeProduct = () => {
        setSelectedProducts((prev) => prev.filter((item) => item.id !== id || item.productType !== productType));
    };
    ...
    const handleDecrease = () => {
        setSelectedProducts((prev) =>
            prev.map((item) =>
                item.id === id && item.productType === productType ? { ...item, quantity: item.quantity - 1 } : item
            )
        );
    };
